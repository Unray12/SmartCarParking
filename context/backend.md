...
- **`YoloOnnxPlateRecognizer`** (mặc định, `.env`): YOLOv5 ONNX 2 giai đoạn — detect vùng biển → deskew (Hough) → detect TỪNG KÝ TỰ (30 class) → tự nhận 1 dòng/2 dòng → ghép qua `normalize_plate`. Output ONNX raw KHÔNG có NMS → tự decode + NMS numpy. Model ở `AI_MODELS_DIR` (`LP_detector.onnx` + `LP_ocr.onnx`). Xem [changelog.md](changelog.md) 2026-07-14 ANPR, 2026-07-21 (4 đợt: tối ưu → fix regression → xử lý ảnh + fix nguyên nhân gốc "NONE" → **fix bug nghiêm trọng chặn nhầm phần lớn biển xe máy**).
  - **Chỉ OCR top-N box theo confidence** (`plate_detector_max_boxes`=2): 1 camera = 1 làn xe, chỉ OCR N box tự tin nhất theo det_conf — KHÔNG lọc theo kích thước pixel (đã thử rồi bỏ: biển ở xa/camera zoom rộng hợp lệ dù nhỏ, lọc cứng theo pixel làm bỏ sót biển thật tùy setup camera). Đây là lớp bảo vệ DUY NHẤT chống biển rác từ box nhiễu (đã bỏ filter "chuỗi rác" - xem dưới).
  - **⚠️ ĐÃ BỎ filter "chuỗi ≥6 số liên tiếp là rác"** (thêm ở đợt tối ưu đầu, tưởng an toàn) - SAI: format biển máy phổ biến "2 số + 1 chữ + 1 số + 5 số" (vd "29Y3-03658" → chuẩn hoá "29Y303658") có ĐÚNG 6 số liên tiếp ngay sau chữ cái, bị chặn nhầm hàng loạt. Đây là nguyên nhân chính khiến độ nhạy "giảm so với ban đầu" mà người dùng báo — không phải do OCR/deskew kém. Bài học: quy tắc suy luận theo format (không phải đo bằng dữ liệu thật đủ đa dạng) rất dễ sai với biển số VN (nhiều sub-format).
  - **Ngưỡng độ dài biển tối thiểu nâng 4→7 ký tự** (`_MIN_PLAUSIBLE_CHARS`, đối chiếu repo tham khảo `trungdinh22/License-Plate-Recognition` cũng hard-reject <7 ký tự) - chặn kết quả THIẾU ký tự (crop mờ) như "9A0"/"09"/"0" lọt qua thành biển giả.
  - `_read_plate_text` luôn thử ĐỦ CẢ 2 candidate (raw + deskewed) như code gốc, SAU ĐÓ nếu vẫn < 7 ký tự thử thêm candidate tăng cường: `_enhance_for_ocr` (CLAHE trên kênh L + upscale CUBIC nếu crop nhỏ) rồi tính lại góc lệch + deskew TRÊN BẢN ĐÃ TĂNG CƯỜNG (crop gốc tương phản kém khiến Hough ước lượng sai góc lệch, CLAHE làm rõ cạnh viền chữ mới ước lượng đúng được). Đối chiếu repo tham khảo: họ dùng lưới 2×2 (contrast on/off × 2 cách chọn line Hough) - đã tự so sánh cách chọn-1-line-duy-nhất của họ với cách lấy trung bình mọi line của mình trên ảnh thật: cách của mình ỔN ĐỊNH HƠN (cách họ ra góc ảo like -90°/82° trên vài ảnh) - KHÔNG đổi phần này.
  - **`test_camera_ai()` (camera_stream.py) thử lại trên vài frame MỚI** (`plate_detect_retry_attempts`=3, cách nhau `plate_detect_retry_interval_seconds`=0.3s) nếu frame đầu không đọc được biển, thay vì bỏ cuộc ngay sau 1 frame — nguyên nhân gốc phần lớn lượt quẹt bị "NONE": không phải model kém, mà 1 frame đơn lẻ dễ bị mờ tạm thời trong khi frame chụp cách đó vài trăm ms đọc đúng hoàn toàn.
