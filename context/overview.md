...
5. **Auth + bảo vệ API bằng JWT:**
   ...
   - **Public (không cần token):** `POST /api/v1/auth/login`, `POST /api/v1/auth/reset-password`, `GET /api/v1/snapshots/files/...` (serve ảnh — `<img>`/`<a>` không gửi được header Authorization), `POST /api/v1/streaming/mediamtx-auth` (MediaMTX gọi server-to-server). **WebSocket `/ws/cameras/{id}` để mở** (không version) nhưng validate JWT/token nội bộ qua query `?token=` trước khi accept.
...
- ⚠️ Bug pre-existing đã biết: xóa camera đang được `parking_lots`/`parking_sessions` tham chiếu có thể crash 500 (`cameras/service.py:delete_camera` chưa có try/except như `delete_parking_lot` đã fix). Xem [changelog.md](changelog.md) 2026-07-14.
